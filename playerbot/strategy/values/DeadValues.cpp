...
GuidPosition GraveyardValue::Calculate()
{
    ...
    else if (getQualifier() == "travel")
    {
        ...
        {
            bool isNullTravelTarget = travelTarget->isNullDestination() || travelTarget->getDestination() == sTravelMgr.nullTravelDestination;

            if (!isNullTravelTarget && travelTarget->getPosition() && travelTarget->getPosition()->getMapId() == bot->GetMapId())
            {
